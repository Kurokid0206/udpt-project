...
async def get_list_label_sentence(
    ...
    skip = page * 100
    label_sentences = label_sentence_repository.get_multi(db=session, skip=skip, limit=100)
...
async def get_list_label_of_sentence(
    ...
    label_sentences = label_sentence_repository.get_by_column(db=session, column='sentence_id', value=id)
    return label_sentences
